...
import collections
import sys
# from Models.Transformer.ViT_adapters import AdapterDWCNN
sys.path.append('/ubc/ece/home/ra/grads/siyi/Research/skin_lesion_segmentation/AViT/')
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, adapt_method=None):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, out_indices=[-1], num_classes=1000, adapt_method=None,drop_rate=0.1):
        '''
        ...
        self.layer1 = self._make_layer(block, 64, layers[0],adapt_method=adapt_method)
        if 2 in out_indices:
            self.layer2 = self._make_layer(block, 128, layers[1], stride=2,adapt_method=adapt_method)
        else:
            self.layer2 = nn.Identity()
        if 3 in out_indices:
            self.layer3 = self._make_layer(block, 256, layers[2], stride=2,adapt_method=adapt_method)
        else:
            self.layer3 = nn.Identity()
        self.drop = nn.Dropout2d(drop_rate)
        if 4 in out_indices:
            self.layer4 = self._make_layer(block, 512, layers[3], stride=2,adapt_method=adapt_method)
        if -1 in out_indices:
    ...
    def _make_layer(self, block, planes, blocks, stride=1,adapt_method=None):
        downsample = None
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, adapt_method=adapt_method))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes,adapt_method=adapt_method))
...
if __name__=='__main__':
    model = resnet34(pretrained=True, out_indices=[0,1,2,3,4], adapt_method=None)  # 11m
